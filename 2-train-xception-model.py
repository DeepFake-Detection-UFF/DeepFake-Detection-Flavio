...
# Função para criar o modelo Xception
def create_xception_model():
    base_model = Xception(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
    for layer in base_model.layers[:-30]:  # Descongelar últimas 30 camadas para fine-tuning
...
def train_xception_model(real_train_path, fake_train_path, real_test_path, fake_test_path, fake_name):
    ...
    log_file = f"logs_xception/log_treinamento_{fake_name}.txt"
    os.makedirs("logs_xception", exist_ok=True)
    with open(log_file, "w") as f:
        f.write(f"Treinamento do modelo Xception para FAKE = {fake_name}\n\n")
    ...
    # Salvar modelo
    os.makedirs("modelos_xception", exist_ok=True)
    model.save(f"modelos_xception/xception_{fake_name}.h5")
