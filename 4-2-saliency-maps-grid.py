...
def generate_saliency_map(model, img_preprocessed):
    try:
        ...
        grads = tf.abs(grads)
        saliency = np.max(grads[0], axis=-1)
        saliency = (saliency - np.min(saliency)) / (np.max(saliency) - np.min(saliency) + 1e-8)
        return saliency
    except Exception as e:
        print(f"[ERRO] Falha ao gerar saliency map: {str(e)}")
        return None
...
def generate_saliency_grid():
    ...
    fig.suptitle("Saliency Maps for Different Deepfake Techniques (Faces)", fontsize=16)
    ...
    for i, (image_name, (img_rgb, img_preprocessed)) in enumerate(selected_images):
        ...
        # Gerar e exibir saliency maps para cada técnica
        for j, technique in enumerate(subfolders[:-1]):
            if technique in models:
                saliency = generate_saliency_map(models[technique], img_preprocessed)
                if saliency is not None:
                    axes[i, j].imshow(img_rgb)
                    axes[i, j].imshow(saliency, cmap='hot', alpha=0.5)
                    axes[i, j].axis('off')
                else:
                    axes[i, j].text(0.5, 0.5, "Erro no saliency map", ha='center', va='center')
                    axes[i, j].axis('off')
            else:
    ...
    plt.savefig('saliency_grid_faces.png')
    plt.show()

# Executar
if __name__ == "__main__":
    generate_saliency_grid()