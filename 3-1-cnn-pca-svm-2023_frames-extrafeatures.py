...
def generate_model(real_train_path, real_test_path, fake_train_path, fake_test_path, fake_name):
    ...
    # --- Pre-trained Model (SVM with PCA) ---
    ...
    print(f"\n=== Avaliação Pre-treinado para FAKE = {fake_name} ===")
    ...
    plot_confusion_matrix(y_test, y_pred_svm, f"Matriz de Confusão Pre-treinado - {fake_name}")
    ...
    with open(log_path, "w") as f:
        ...
        f.write(f"F1-score (macro): {f1_svm:.4f}\n")
        f.write(f"AUC: {auc_svm:.4f}\n" if auc_svm else "AUC: N/A\n")
