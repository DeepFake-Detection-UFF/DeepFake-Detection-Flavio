...
def create_resnet_model():
    ...
    for layer in base_model.layers[:-25]:  # Descongelar últimas 25 camadas para fine-tuning
...
def train_resnet_model(real_train_path, fake_train_path, real_test_path, fake_test_path, fake_name):
    ...
    log_file = f"logs_resnet/log_treinamento_{fake_name}.txt"
    os.makedirs("logs_resnet", exist_ok=True)
    with open(log_file, "w") as f:
        f.write(f"Treinamento do modelo ResNet50 para FAKE = {fake_name}\n\n")
    ...
    # Salvar modelo
    os.makedirs("modelos_resnet", exist_ok=True)
    model.save(f"modelos_resnet/resnet_{fake_name}.h5")
